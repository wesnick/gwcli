...
OAuth credentials live in `~/.config/gwcli/`; never commit those JSON files or redact sensitive IDs in examples. Prefer environment variables or files under the user’s home config directory when writing docs or scripts. If you touch crypto or token-handling code in `pkg/gpg/`, add tests that exercise invalid key material and update the README if new scopes or secrets are required.
