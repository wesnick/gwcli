...
func New(configDir string, userEmail string, verbose bool) (*CmdG, error) {
	...
	if isServiceAcct {
		...
		serviceAcctAuth, err := NewServiceAccountAuthenticator(credFile2, userEmail)
		if err != nil {
		...
		serviceAcctAuth2, err := NewServiceAccountAuthenticator(credFile3, userEmail)
		if err != nil {
...
// LoadLabels batch loads all labels into the cache directly from the
// Gmail API, plus the Gmail built-in system labels.
func (c *CmdG) LoadLabels(ctx context.Context, verbose bool) error {
	...
	// Add all labels returned by the Gmail API. System labels already seeded
	// above keep their friendly names; user labels are added as-is.
	userCount := 0
	for _, gl := range gmailLabels {
		if _, isSystem := systemLabels[gl.Id]; isSystem {
			continue
		}
		c.labelCache[gl.Id] = &Label{
			ID:       gl.Id,
			Label:    gl.Name,
			Response: gl,
		}
	...
	log.Infof("Loaded %d labels (%d user, %d system) in %v",
		len(c.labelCache), userCount, len(systemLabels), time.Since(st))
...
// Labels are lazily loaded from the Gmail API on first access.
