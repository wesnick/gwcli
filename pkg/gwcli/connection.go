...
// minimalConfigTemplate is the starter config.jsonnet written on first run
// when none exists. It is intentionally empty: system labels are added
// automatically, and users can populate labels/rules or run
// 'gwcli gmailctl download' to import their account's existing setup.
const minimalConfigTemplate = `// Auto-generated by gwcli on first run.
// System labels (INBOX, TRASH, etc.) are always available.
// Add custom labels/rules below, or run 'gwcli gmailctl download'
// to import your account's existing filters and labels.
{
  version: '` + gmailctl.Version + `',
  labels: [],
  rules: [],
}
`

// writeMinimalConfig creates a starter config.jsonnet at configPath,
// creating the parent directory if needed. It never overwrites an
// existing file.
func writeMinimalConfig(configPath string) error {
	...
	if err := os.WriteFile(configPath, []byte(minimalConfigTemplate), 0o600); err != nil {
...
func (c *CmdG) LoadLabels(ctx context.Context, verbose bool) error {
	...
	if err != nil {
		if err == gmailctl.ErrNotFound {
			if werr := writeMinimalConfig(configPath); werr != nil {
				return fmt.Errorf("config.jsonnet not found at %s and could not auto-create it: %w", configPath, werr)
			}
			fmt.Fprintf(os.Stderr, `No config.jsonnet found - created a minimal one at:
  %s

System labels (INBOX, TRASH, etc.) work out of the box. To manage custom
labels and filters, edit that file or import your account's existing setup:
