...
import (
	...
	"github.com/wesnick/gwcli/pkg/gpg"
)
...
func (msg *Message) annotateAttachments() error {
	...
	for _, p := range msg.Response.Payload.Parts {
		...
		bodystr = append(bodystr, fmt.Sprintf("\n<<<Attachment %q>>>", p.Filename))
	}
...
func (l *Label) LabelString() string {
	...
	// Note: Color formatting removed for CLI use
	// Labels are returned as plain text
	if c != "" {
		return fmt.Sprintf("%s%s", c, l.Label)
	}
	return l.Label
}
...
func makeBodyAlt(ctx context.Context, part *gmail.MessagePart, preferHTML bool) (string, error) {
	...
	for _, p := range part.Parts {
		...
		// Note: HTML parts are kept as raw HTML (no rendering)
		// Downstream consumers can pipe to html2md or other tools if needed
...
func (msg *Message) tryGPGInlineSigned(ctx context.Context) error {
	...
	b2 := inlineGPG.ReplaceAllStringFunc(msg.body, func(in string) string {
		...
		return fmt.Sprintf("BEGIN message signed by %s\n%s\nEND message signed by %s", st.Signed, in, st.Signed)
	})
	if e2 != nil {
		return e2
	}
...
func (msg *Message) load(ctx context.Context, level DataLevel) error {
	...
	// Only process headers if Payload is available (LevelMetadata or LevelFull)
	if msg.Response.Payload != nil && msg.Response.Payload.Headers != nil {
	...
	if level == LevelFull {
		...
		if err := msg.tryGPGEncrypted(ctx); err != nil {
			msg.body = fmt.Sprintf("ERROR Decrypting GPG: %v", err)
		}
