...
func runGmailctlDownload(ctx context.Context, configDir, userEmail, outputFile string, skipConfirm bool, out *outputWriter) error {
	// Get config paths
	...
	if targetPath == "" {
		targetPath = paths.Config
	} else if targetPath != "-" && !filepath.IsAbs(targetPath) {
		targetPath = filepath.Join(paths.Dir, targetPath)
	}

	if targetPath != "-" {
		if _, statErr := os.Stat(targetPath); statErr == nil && !skipConfirm {
